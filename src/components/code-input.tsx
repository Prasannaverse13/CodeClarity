...
export function CodeInput({ onExplanationUpdate }: CodeInputProps) {
  ...
  const [isLoading, setIsLoading] = useState(false); // Local loading state for the component's UI
  ...
  const handleSubmit = async () => {
    ...
    // 🧭 1. Intent Confirmation & Context Detection (Initial Status sent to parent)
    onExplanationUpdate(null, true, null, "Received! Analyzing your code...");

    try {
      // ⚙️ 2. Code Analysis in Steps (Simulated via API call - Status sent to parent)
      ...
      // Success: Update parent with result and final status
      const detectedLang = result.language && result.language !== "Unknown"
        ? `Detected language: ${result.language}. `
        : '';
      onExplanationUpdate(result, false, null, `${detectedLang}Analysis complete.`); // Send successful result and update parent loading state
    } catch (error) {
      console.error('Error explaining code:', error);
      const errorMsg = error instanceof Error ? error.message : 'An unexpected error occurred.';
      onExplanationUpdate(null, false, errorMsg, "Agent encountered an error during analysis."); // Send error message and update parent loading state
       toast({
      ...
      // Ensure local loading state is reset regardless of success or error
      setIsLoading(false);
    }
  };
  ...
  const handleClear = () => {
    ...
    // Also notify parent to clear its state and set its loading to false
  ...
  return (
    <div className="flex flex-col h-full">
      <Textarea
        ...
        disabled={isLoading} // Disable based on local loading state
